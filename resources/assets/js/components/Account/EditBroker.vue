            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-lg-8">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active"><strong>编辑客户经理</strong></li>
                    </ol>
                </div>
                <div class="col-lg-4">
                    ...
                    <div class="ibox-content col-lg-12">
                        <form enctype="multipart/form-data" class="form-horizontal">
                            ...
                            <div class="form-group col-lg-6">
                                ...
                                <div class="col-lg-8">
                                    <select class="form-control" v-model="broker.operator_code" v-if="permission.can('account.broker.delete')">
                                        <option v-for="item in operators" :value="item.code">{{item.name}}</option>
                                    </select>
                                    <select v-else class="form-control" v-model="broker.operator_code" disabled="disabled">
                                        <option v-for="item in operators" :value="item.code">{{item.name}}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group col-lg-6">
                                ...
                                <div class="col-lg-8">
                                    <select class="form-control" v-model="broker.branch_id" v-if="permission.can('account.broker.delete')">
                                        <option v-for="item in branches" :value="item.id">{{item.branch_name}}</option>
                                    </select>
                                    <select  v-else class="form-control" v-model="broker.branch_id" disabled="disabled">
                                        <option v-for="item in branches" :value="item.id">{{item.branch_name}}</option>
                                    </select>
                                </div>
                            </div>
...
<script>
  export default{
    ...
    methods: {
      ...
      preservation(){
        axios.post('/api/broker/edit', this.broker).then(response => {
      ...
      getBroker(){
        axios.get('/api/broker/edit/'+this.$route.params.id).then(response => {
